...
for i =1:M/8
    for j=1:N/8
        ...
        if mean2(I1(8*i-7:8*i,8*j-7:8*j))~=0 && wavelengths(i,j) < 1000
            ...
            gaborfilter = generate_gabor(16,-180*orientation_new(i,j)/pi,wavelengths(i,j));
            I_result(8*i-7:8*i,8*j-7:8*j) = imfilter(I1(8*i-7:8*i,8*j-7:8*j),gaborfilter);
        end
    end
end
I1 = I1 +pic2;
...
I_result_bin=imbinarize(I_result,0.7); 
...
imwrite(I_result_smooth,'.\1_result.bmp');